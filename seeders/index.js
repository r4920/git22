...
async function seedUser () {
  try {
    ...
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Hulda16' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'ERuWFOVK_lrRxcq',
      'username':'Alec.Huel99',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Alec.Huel99' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
